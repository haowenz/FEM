...
int load_index() {
  ...
  fprintf(stderr, "Max hash value: %d, number of reference sequences: %d, number of bases: %d, load in %fs!\n", max_hash_value, reference.refNum, reference.lookupTable[reference.refNum] - reference.lookupTable[0],  get_real_time() - startTime);
  return tmp;
}
