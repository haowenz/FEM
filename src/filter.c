...
void gNewOptPrefixGrams(uint32_t readLen,uint32_t gramLen,gCandidate *optCandidates, gCandidate *swapOptCandidates, uint32_t *totalCandidatesNum){
...
void vlNewOptPrefixGrams(uint32_t readLen,uint32_t gramLen,vlCandidate *optCandidates, vlCandidate *swapOptCandidates, uint32_t *totalCandidatesNum){
    uint32_t nGrams = readLen - gramLen + 1;
    //if(nGrams > invertedLists.size()){ cerr << "Assertion Failed: not enough inverted lists" << endl; exit(1); }
    int nSeeds = edit_distance + 2;
...
uint32_t groupSeedingCandidateGenerator(const Read *read, const int reverseComplemented, uint32_t **candidatesList,uint32_t **swapCandidatesList, uint32_t *candidatesNumMax, TwoTuple **candis, TwoTuple **tempCandis, uint32_t *tupleNum,uint32_t *candiNumWithoutAddFilter) {
    ...
    *candiNumWithoutAddFilter=0;
    ...
    for(int si=0;si<step_size;++si){
        ...
        gOptPrefixGrams(coveredNum, totalSubSeedNum, optCandidates, swapOptCandidates,&totalPositionNum);

        //gNewOptPrefixGrams(totalSubSeedNum,coveredNum,optCandidates,swapOptCandidates,&totalPositionNum);

        totalCandidatesCount += totalPositionNum;

        *candiNumWithoutAddFilter=totalCandidatesCount;
        qsort(swapOptCandidates, edit_distance + 1 + additional_gram_num, sizeof(gCandidate), comparegCandidate);
...
uint32_t variableLengthSeedingCandidateGenerator(const Read *read, const int reverseComplemented, uint32_t **candidatesList,uint32_t **swapCandidatesList, uint32_t *candidatesNumMax, TwoTuple **candis, TwoTuple **tempCandis, uint32_t *tupleNum,uint32_t *candiNumWithoutAddFilter) {
	const uint8_t *bases = read->bases;
    if (reverseComplemented == 1) {
        bases = read->rc_bases;
    }
...
//dp for seed selection start
    ...
    //vlNewOptPrefixGrams(read->length,seedLengthMin,optCandidates, swapOptCandidates, &totalPositionNum);
    
    vlOptPrefixGrams(seedLengthMin, read->length, optCandidates, swapOptCandidates,&totalPositionNum);
    ...
    *candiNumWithoutAddFilter=totalCandidatesCount;
    ...
    qsort(minSubSeeds, (edit_distance+additional_gram_num+1)*step_size, sizeof(SubSeed), compareSubSeed);
